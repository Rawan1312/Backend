...
public class AppDBContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<User>(entity =>
        {
            ...
            entity.HasMany(o => o.Order)
                  .WithOne(u => u.User)
                  .HasForeignKey(u => u.UserId)
                  .OnDelete(DeleteBehavior.Cascade);

            entity.HasMany(p => p.Payment)
                  .WithOne(u => u.User)
                  .HasForeignKey(u => u.UserId)
                  .OnDelete(DeleteBehavior.Cascade);

            entity.HasMany(adr => adr.Address)
                  .WithOne(u => u.User)
                  .HasForeignKey(u => u.UserId)
                  .OnDelete(DeleteBehavior.Cascade);
        });
        ...
        // Category entity configuration with cascading delete for Products
        modelBuilder.Entity<Category>()
            .HasMany(p => p.Products)
            .WithOne(c => c.Category)
            .HasForeignKey(c => c.CategoryId)
        ...
        // Shipment entity configuration and Order relationship
        modelBuilder.Entity<Shipment>(entity =>
        {
            entity.HasKey(e => e.ShipmentId);
            entity.Property(e => e.ShipmentId).HasDefaultValueSql("uuid_generate_v4()");
