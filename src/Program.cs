...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowSpecificOrigins", builder =>
    {
        builder.WithOrigins("http://localhost:5173") // النطاقات المسموح بها
               .AllowAnyMethod() // يسمح بجميع الطرق
               .AllowAnyHeader() // يسمح بجميع الرؤوس
               .AllowCredentials(); // يسمح بالاعتمادات
    });
});

// إعدادات JWT
var jwtKey = Environment.GetEnvironmentVariable("Jwt__Key") ?? throw new InvalidOperationException("JWT Key is missing.");
var jwtIssuer = Environment.GetEnvironmentVariable("Jwt__Issuer") ?? throw new InvalidOperationException("JWT Issuer is missing.");
var key = Encoding.ASCII.GetBytes(jwtKey);

// إعداد Entity Framework Core مع Npgsql
builder.Services.AddDbContext<AppDBContext>(options =>
...
.AddJwtBearer(options =>
{
    ...
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        ValidateAudience = true,
        ValidIssuer = jwtIssuer,
        ValidAudience = Environment.GetEnvironmentVariable("Jwt__Audience") ?? throw new InvalidOperationException("JWT Audience is missing.")
    };
});
...
app.UseHttpsRedirection();
app.UseCors("AllowSpecificOrigins"); // تطبيق سياسة CORS
app.UseAuthentication();
app.UseAuthorization();
...
app.UseSwaggerUI(c => 
{
    ...
    c.RoutePrefix = string.Empty; // اجعل Swagger UI في الجذر
});
